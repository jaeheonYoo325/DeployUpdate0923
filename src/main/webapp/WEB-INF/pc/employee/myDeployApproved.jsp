			        				...
			        				<c:choose>
										<c:when test="${not empty deployApproved}">
											<c:forEach items="${deployApproved}" var="deployApproved">
												<tr>													
													<td>${deployApproved.deployNo}</td>
													<td>${deployApproved.deployDrafterName}</td>
													<td>${deployApproved.deployDraftDate}</td>
													<td>${deployApproved.codeName}</td>
													<td>${deployApproved.deployApprovalRequestDate}</td>
													<td>${deployApproved.deployApprovalLineName}</td>
													<td>${deployApproved.deployApprovalDate}</td>
													<td>${deployApproved.deployApprovalLineConfirmName}</td>
													<td>
														<c:choose>
															<c:when test="${deployApproved.isDeployApproval eq 'Y'}">
																승인
															</c:when>
															<c:otherwise>
																반려
													...
													<td><input type="button" value="상세내역" class="btn btn-primary" onclick="openPopup(${deployApproved.deployNo},'myDeploy')"></td>
<%-- 													<td><input type="button" value="상세내역" class="btn btn-primary" onclick="showApprovalDetail(${deployApproved.deployNo},'myDeploy')"></td> --%>
												</tr>
											</c:forEach>
										</c:when>
										<c:otherwise>
